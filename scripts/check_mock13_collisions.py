"""Check (question_text, option_a) collisions between MDCAT_MOCK_13.json
and every other parsed-mcqs JSON (past papers + Mocks 1-12)."""
import json
from pathlib import Path

DIR = Path(__file__).parent.parent / "mdcat-content" / "parsed-mcqs"
TARGET = DIR / "MDCAT_MOCK_13.json"
...
other_files = sorted(
    ...
    if p.name != "MDCAT_MOCK_13.json" and p.suffix == ".json" and "checkpoint" not in p.name
)
