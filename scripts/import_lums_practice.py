"""One-off: import the LUMS Math/Verbal standalone practice export (see
export_lums_practice.py) back into a DB, content-matched (question_text +
option_a) so it's safe to run more than once without creating duplicates.

Creates any missing Subject/Topic/Subtopic rows it needs along the way.

Usage (from backend/, venv active, ON LOCAL):
    python scripts/import_lums_practice.py lums_practice_export.json
    python scripts/import_lums_practice.py lums_practice_export.json --dry-run
"""
import argparse
import json
import os
