"""Convert LCAT Mock 2 (.py) into importer-ready JSON.

Source: lums-content/raw-sources/lums_mock_02.py — a ParhaiKarlo-written
full-length 120-Q mock (7 sections, Verbal 69 / Math 51). Not an official
LUMS paper and no official answer key exists for it.

Two deliberate differences from convert_lums_sample_to_json.py:

  - needs_review is False, so import_mcqs.py lands every row with
    is_verified=True. The source file's own docstring says NEEDS_REVIEW=True
    ("answer keys prepared by content team, pending human verification"), but
    that verification has since happened: on 2026-09-12 all 120 keys were
    independently re-derived (Verbal grammar/logic, Math worked by hand) and
    every option's "Correct" label in the explanation set was cross-checked
    against the source key. That pass also fixed three real defects (a severe
    B-heavy answer-letter skew, an unsupported claim in Q41's credited option,
    and an ungrammatical pronoun antecedent in Q79). This mirrors how the
    LUMS sample paper and the MDCAT mocks — which equally have no external
    key — were promoted to is_verified=True after a content check.
    Unverified questions are invisible to /api/quiz/questions/, so leaving
    the flag on would import the mock and then hide it.
...
quiz.MockTest row via scripts/attach_lums_mock2.py (the same home MDCAT
...
from lums_mock_02 import QUESTIONS  # noqa: E402
...
def main():
    ...
    for q in QUESTIONS:
        out.append({
            ...
            "explanation": None,
            "needs_review": False,  # keys content-verified 2026-09-12
            "source_file": "lums-content/raw-sources/lums_mock_02.py",
            "topic": q["topic"],
            "subtopic": q["subtopic"],
            "tag_confidence": "high",
    ...
    out_path = ROOT / "lums-content" / "parsed-mcqs" / "LUMS_MOCK_2.json"
