"""One-off: mdcat_mock_20.py's answer key was skewed toward 'B'
(121/180) and away from 'C'/'D' (20/1). Rebalance each question's option
order (swap the correct option into a target letter slot, round-robin
A/B/C/D) so the correct-answer distribution comes out 45/45/45/45,
then rewrite the file in the same dict-literal style. Has 5 image-based
questions, so the "image" key is preserved on rewrite.
"""
import ast
from pathlib import Path
from collections import Counter

SRC = Path(__file__).parent.parent / "mdcat-content" / "mdcat_mock_20.py"
...
assert all(v == 45 for v in balance.values()), balance
